...
export type TrainingDetailScope = {
...
interface Props {
  trainingId: string;
  scope?: TrainingDetailScope;
}

export function TrainingDetailView({ trainingId, scope }: Props) {
  const id = trainingId;
  const effectiveScope: TrainingDetailScope = scope ?? {
    breadcrumbItemsPrefix: [{ label: "Treinos", href: "/trainings" }],
    fallbackReturnHref: "/trainings",
  ...
  const [returnHref] = useState(() =>
    getReturnTo(effectiveScope.returnToKey, effectiveScope.fallbackReturnHref),
  );
  ...
  async function handleDelete() {
    ...
    try {
      ...
      router.push(effectiveScope.fallbackReturnHref);
    } catch {
  ...
  if (!session) {
    return (
      <div className="p-4 md:p-8 max-w-2xl mx-auto text-center py-16">
        ...
        <Button variant="outline" className="mt-4" onClick={() => router.push(effectiveScope.fallbackReturnHref)}>Voltar aos treinos</Button>
      </div>
    );
  }
