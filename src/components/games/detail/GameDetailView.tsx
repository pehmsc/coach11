...
export type GameDetailScope = {
  /** Items que prefixam o breadcrumb. */
  breadcrumbItemsPrefix: BreadcrumbItem[];
  /** URL fallback "Voltar aos jogos". */
  fallbackReturnHref: string;
  /** Chave do useReturnTo. */
  returnToKey: string;
  /** Label do StickyBackLink. */
  backLabel: string;
  /** URL base para sub-rotas deste jogo (summary). Default: /games/[id]. */
  gameBaseHref: string;
};
...
export function GameDetailView({ gameId, scope }: Props) {
  const id = gameId;
  const effectiveScope: GameDetailScope = scope ?? {
    breadcrumbItemsPrefix: [{ label: "Jogos", href: "/games" }],
    fallbackReturnHref: "/games",
    returnToKey: "games",
    ...
    return (
      <div className="p-4 md:p-8 text-center py-16">
        ...
        <div className="mt-4 flex justify-center">
          <StickyBackLink
            href={effectiveScope.fallbackReturnHref}
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto">
      ...
      {game.status === "completed" && (
        <CompletedResult
          game={game}
          onViewSummary={() => router.push(`${effectiveScope.gameBaseHref}/summary`)}
        />
      )}
