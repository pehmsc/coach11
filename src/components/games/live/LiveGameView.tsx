...
/**
 * Scope override permite a rota escalao
 * (/teams/[ageGroupId]/games/[gameId]/live) personalizar breadcrumb,
 * back-link e redirect destinos sem duplicar a view inteira.
 */
export type LiveGameScope = {
  /**
   * Items que prefixam o breadcrumb. Tipicamente termina em "Jogos"
   * (link a lista de jogos do contexto — global ou escalao).
   */
  breadcrumbItemsPrefix: BreadcrumbItem[];
  /**
   * URL base do detalhe do jogo (ex: `/games/[id]` ou
   * `/teams/[id]/games/[gameId]`). Usado para:
   * - href do StickyBackLink ("Voltar ao jogo")
   * - href do crumb "vs Opponent" no breadcrumb
   * - backHref do LiveLockedState
   * - base para o summaryHref (`${gameBaseHref}/summary`) propagado ao hook
   */
  gameBaseHref: string;
  /** Chave do useReturnTo (sessionStorage). */
  returnToKey: string;
...
export function LiveGameView({ gameId, scope }: Props) {
  const state = useLiveGameState(gameId, {
    summaryHref: `${scope.gameBaseHref}/summary`,
  });
  const ageGroupMeta = useAgeGroupMeta(state.game?.age_group_id ?? null);
  const [returnHref] = useState(() =>
    getReturnTo(scope.returnToKey, scope.gameBaseHref),
  );
  ...
  if (!state.isFinalized && !liveUnlocked) {
    return <LiveLockedState backHref={scope.gameBaseHref} />;
  }

  const opponentCrumbLabel = state.game.opponent_name
      ...
      >
        <Breadcrumb
          items={[
            ...
            { label: opponentCrumbLabel, href: scope.gameBaseHref },
            { label: "Live" },
          ]}
        />
      </StickyBackLink>
