...
/**
 * Scope override permite à rota escalão (/teams/[id]/players/[playerId]/games)
 * personalizar breadcrumb e returnHref sem duplicar a view inteira.
 */
export type PlayerGamesHistoryScope = {
...
interface Props {
  playerId: string;
  scope: PlayerGamesHistoryScope;
}

export function PlayerGamesHistoryView({ playerId, scope }: Props) {
  ...
  async function load(offset: number, append: boolean) {
    ...
    try {
      const res = await fetch(
        `/api/players/${playerId}/games?limit=${PAGE_SIZE}&offset=${offset}`,
      );
