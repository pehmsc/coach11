"use client";

import Link from "next/link";
import { useSearchParams } from "next/navigation";
import { useEffect, useMemo, useState } from "react";
import { createClient } from "@/lib/supabase/client";
import { Button } from "@/components/ui/button";
import { Loader2 } from "lucide-react";

type InviteInfo = {
  clubName?: string;
  ageGroupName?: string;
  role?: string;
  invitedBy?: string;
};

const ROLE_LABELS: Record<string, string> = {
  head_coach: "Treinador Principal",
  assistant_coach: "Treinador Adjunto",
  intern_coach: "Treinador Estagiário",
  goalkeeper_coach: "Treinador de Guarda-Redes",
  fitness_coach: "Preparador Físico",
  physiotherapist: "Fisioterapeuta",
  doctor: "Médico",
  analyst: "Analista / Observador",
  team_manager: "Team Manager",
  age_group_coordinator: "Coordenador de Escalão",
  club_coordinator: "Coordenador de Clube",
};

export default function InviteClient() {
  const sp = useSearchParams();
  const supabase = useMemo(() => createClient(), []);

  const raw =
    sp.get("code") ?? sp.get("inviteCode") ?? sp.get("invite_code") ?? "";
  const code = decodeURIComponent(raw).trim().toUpperCase();
  const email = decodeURIComponent(sp.get("email") ?? "").trim();

  const [info, setInfo] = useState<InviteInfo | null>(null);
  const [loadingInfo, setLoadingInfo] = useState(true);
  const [googleLoading, setGoogleLoading] = useState(false);

  // Guardar código em localStorage e sessionStorage imediatamente
  useEffect(() => {
    if (code) {
      localStorage.setItem("inviteCode", code);
      sessionStorage.setItem("pending_invite_code", code);
    }
    if (email) {
      localStorage.setItem("inviteEmail", email);
      sessionStorage.setItem("pending_invite_email", email);
    }
  }, [code, email]);

  // Buscar info do convite
  useEffect(() => {
    if (!code) {
      setLoadingInfo(false);
      return;
    }

    async function fetchInfo() {
      try {
        const res = await fetch(`/api/invite/info?code=${encodeURIComponent(code)}`);
        if (res.ok) {
          const data = await res.json();
          setInfo(data);
        }
      } catch {
        // Não bloquear a página se falhar
      } finally {
        setLoadingInfo(false);
      }
    }

    void fetchInfo();
  }, [code]);

  async function handleGoogleLogin() {
    setGoogleLoading(true);

    // Definir cookie HTTP-only ANTES do redirect OAuth
    // Garante que o invite_code sobrevive a qualquer cadeia de redirects
    await fetch("/api/invite/set-pending-cookie", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, email }),
    }).catch(() => null);

    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || window.location.origin;
    const callbackUrl = new URL("/auth/callback", baseUrl);
    callbackUrl.searchParams.set(
      "next",
      `/dashboard?code=${encodeURIComponent(code)}`,
    );
    callbackUrl.searchParams.set("invite_code", code);
    if (email) callbackUrl.searchParams.set("invite_email", email);

    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: callbackUrl.toString(),
      },
    });

    if (error) {
      setGoogleLoading(false);
    }
  }

  if (!code) {
    return (
      <main className="mx-auto max-w-md p-6">
        <h1 className="text-xl font-semibold">Convite inválido</h1>
        <p className="mt-2 text-sm text-slate-600">
          O link não traz o código. Pede ao coordenador para reenviar.
        </p>
        <Link className="mt-4 inline-block underline" href="/login">
          Ir para login
        </Link>
      </main>
    );
  }

  const qs = new URLSearchParams();
  qs.set("code", code);
  if (email) qs.set("email", email);

  return (
    <main className="mx-auto max-w-md p-6">
      <h1 className="text-2xl font-extrabold tracking-tight">
        Convite <span className="text-emerald-500">Coach11</span>
      </h1>

      <div className="mt-5 rounded-2xl border bg-white p-4 space-y-2">
        {loadingInfo ? (
          <div className="flex items-center justify-center py-4">
            <Loader2 size={20} className="animate-spin text-slate-400" />
          </div>
        ) : info ? (
          <>
            {info.clubName && (
              <div>
                <p className="text-xs text-slate-500">Clube</p>
                <p className="font-semibold text-slate-800">{info.clubName}</p>
              </div>
            )}
            {info.ageGroupName && (
              <div>
                <p className="text-xs text-slate-500">Escalão</p>
                <p className="font-semibold text-slate-800">{info.ageGroupName}</p>
              </div>
            )}
            {info.role && (
              <div>
                <p className="text-xs text-slate-500">Função</p>
                <p className="font-semibold text-slate-800">
                  {ROLE_LABELS[info.role] ?? info.role}
                </p>
              </div>
            )}
            {info.invitedBy && (
              <p className="text-xs text-slate-400 pt-1">
                Convidado por {info.invitedBy}
              </p>
            )}
          </>
        ) : (
          <>
            <p className="text-xs text-slate-500">Código</p>
            <p className="mt-1 font-mono text-lg font-extrabold tracking-widest">
              {code}
            </p>
          </>
        )}
        {email && (
          <p className="text-xs text-slate-500 pt-1">
            Email: <span className="font-medium">{email}</span>
          </p>
        )}
      </div>

      <div className="mt-6 grid gap-3">
        <Button
          onClick={() => void handleGoogleLogin()}
          disabled={googleLoading}
          className="h-12 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-semibold"
        >
          {googleLoading ? (
            <Loader2 size={16} className="animate-spin" />
          ) : (
            <>
              <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Continuar com Google
            </>
          )}
        </Button>

        <Link
          href={`/login?${qs.toString()}`}
          className="rounded-2xl border px-4 py-3 text-center font-semibold hover:bg-slate-50 transition-colors"
        >
          Continuar com email →
        </Link>
      </div>
    </main>
  );
}
