-- Hotfix: preservar status terminal não-completed ao editar stats finais.
--
-- Antes: rpc_finalize_game forçava sempre games.status='completed', o que
-- transformava silenciosamente jogos 'cancelled' em 'completed' quando o user
-- editava stats finais via "Editar Final Stats" (PR #126).
--
-- Agora: status 'cancelled' e 'postponed' são preservados. Outros estados
-- (incluindo 'live' e 'scheduled', usados pelo finalize inicial) continuam a
-- transitar para 'completed' como antes.
...
begin
  ...
  -- Outros estados (scheduled, live) transitam para 'completed' como antes.
