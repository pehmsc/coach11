-- Override total de stats finais com flag de auditoria.
-- Permite editar todos os campos numéricos via UI, mantendo distinção
-- entre stats automáticas (calculadas de game_events) e manuais.

-- 1. Colunas novas em game_final_stats
alter table public.game_final_stats
  add column if not exists edited_manually boolean not null default false,
  add column if not exists updated_at timestamptz not null default now();

comment on column public.game_final_stats.edited_manually is
  'TRUE quando pelo menos um campo NUMÉRICO (minutes_played, goals, own_goals, assists, yellow_cards, red_cards) foi sobrescrito manualmente via UI. FALSE significa que reflecte o cálculo automático a partir de game_events. Reset via UI volta esta flag a false e recomputa. Nota: alterar apenas coach_rating, notes, is_mvp, ou lineup_type NÃO conta como manualidade.';

comment on column public.game_final_stats.updated_at is
  'Timestamp da última alteração da row. Mantido por trigger set_updated_at. Distinto de finalized_at, que regista o momento do finalize inicial.';

-- 2. Trigger updated_at (reusa public.set_updated_at existente)
drop trigger if exists game_final_stats_set_updated_at on public.game_final_stats;
create trigger game_final_stats_set_updated_at
  before update on public.game_final_stats
  for each row
  execute function public.set_updated_at();

-- 3. Replace rpc_finalize_game para aceitar edited_manually no JSONB
...
-- 4. Replace wrapper rpc_recalculate_game_summary_auth com guards relaxados
-- - Permission: user_can_write_game (todo o staff do escalão), antes user_is_game_coordinator.
-- - Status: NOT IN ('scheduled', 'live'), antes só 'completed'.
