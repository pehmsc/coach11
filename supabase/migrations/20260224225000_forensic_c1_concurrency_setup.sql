-- Forensic C1: setup para teste concorrente da rpc_redeem_staff_invite.
...
begin
  ...
  if v_club_a is null then
    raise notice 'A2_RACE_SETUP|status=SKIPPED|reason=no_club';
    return;
  end if;
  ...
  if v_user_a is null then
    raise notice 'A2_RACE_SETUP|status=SKIPPED|reason=no_user';
    return;
  end if;
  ...
  raise notice 'A2_RACE_SETUP|status=READY|club_a=%|user_a=%|user_a_email=%|age_group_r=%|team_r=%|code=%',
    v_club_a, v_user_a, v_user_a_email, v_age_group_r, v_team_r, v_code;
end $$;
