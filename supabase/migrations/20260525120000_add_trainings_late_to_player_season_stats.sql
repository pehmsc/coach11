-- Insights: acrescenta trainings_late a public.player_season_stats.
--
-- A tabela `training_attendance.status` tem o valor 'late' (atrasos) mas a
-- view nao o expunha — apenas present/absent/injured. Esta migration
-- acrescenta o contador dentro da MESMA subconsulta pre-agregada por
-- player_id usada pela correccao do PR #219 (fan-out), preservando o
-- padrao de pre-aggregation e o flag security_invoker=true.
--
-- Nota: PostgreSQL nao permite CREATE OR REPLACE VIEW que altere a ORDEM ou
-- introduza colunas no meio das existentes (SQLSTATE 42P16). Por isso o
-- trainings_late e' colocado NO FIM da view, depois de avg_rating, mantendo
-- todas as outras colunas e tipos inalterados.
...
LEFT JOIN (
  SELECT
    ...
    count(*) FILTER (WHERE status = 'late'::text) AS trainings_late
