-- BUG CRÍTICO: rpc_redeem_staff_invite estava a inserir em team_staff em vez de age_group_staff.
--
-- Causa: a migration 20260323100000_fix_rpc_staff_invite_role_mapping.sql corrigiu o mapeamento
-- de roles (club_coordinator → coordinator; outros → coach) mas acidentalmente reverteu a
-- inserção para team_staff em vez de age_group_staff, desfazendo o trabalho de 20260317100000.
--
-- Consequência: após redeem de convite de staff, o utilizador ficava em team_staff mas não em
-- age_group_staff. O dashboard (RSC) e resolveUserTeamContext verificam age_group_staff — pelo
-- que o staff era redireccionado para /onboarding mesmo após aceitar o convite.
--
-- Correcção: restaurar o insert em age_group_staff (com linked_team_id e aplicação de
-- initial_permissions) mantendo o mapeamento de roles correcto de 20260323100000.
